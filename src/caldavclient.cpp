...
CalDavClient::CalDavClient(const QString& aPluginName,
    ...
    , mCleanSyncMarkersFileDir(QStringLiteral("/home/nemo/.local/share/system/privileged/Sync/"))
    , mCleanSyncMarkersFile(QStringLiteral("/home/nemo/.local/share/system/privileged/Sync/caldav.ini"))
    , mManager(0)
    , mAuth(0)
    , mCalendar(0)
    , mStorage(0)
...
bool CalDavClient::cleanSyncRequired(int accountId)
{
    ...
    ProcessMutex *mutex(ProcessMutex::instance(mProcessMutex, mCleanSyncMarkersFile));
    mutex->lock();

    char *cleaned_value = SailfishKeyProvider_ini_read(
            mCleanSyncMarkersFile.toLatin1(),
            "General",
    ...
    if (!alreadyClean) {
        ...
        if (SailfishKeyProvider_ini_write(
                mCleanSyncMarkersFileDir.toLatin1(),
                mCleanSyncMarkersFile.toLatin1(),
                "General",
                QStringLiteral("%1-cleaned").arg(accountId).toLatin1(),
                "true") != 0) {
